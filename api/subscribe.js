...
const SOURCE_LABEL = {
  ...
  'tonehomes-community': 'Tonehomes — community list',
...
module.exports = async (req, res) => {
  ...
  // Keep a compact context string for newsletter forms and a separately queryable
  // source/campaign trail for Tonehomes inquiries.
  const answers = [body.q1, body.q2, body.q3, body.q4, body.detail].filter(Boolean).join(' | ').slice(0, 250);
  const isTonehomesLead = body.source === 'tonehomes-realtor';
  const tonehomesCampaign = [
    body.utm_campaign && body.utm_campaign !== 'none' ? body.utm_campaign : '',
    body.path ? 'path:' + body.path : '',
    body.cta ? 'cta:' + body.cta : ''
  ].filter(Boolean).join(' | ').slice(0, 250);
  const beehiivSource = isTonehomesLead
    ? String(body.lead_source || 'direct').slice(0, 80)
    : (body.source || 'unknown');
  const beehiivMedium = isTonehomesLead
    ? String(body.utm_medium || 'none').slice(0, 80)
    : body.source === 'note-blankspace' ? 'note_popup'
    ...
    : 'unknown';
  const beehiivCampaign = isTonehomesLead
    ? (tonehomesCampaign || 'tonehomes_inquiry')
    : (answers || 'early_access');

  // Newsletter subscribers get our branded Resend welcome. Tonehomes inquiries
  // are stored as leads but should not receive Beehiiv's default welcome email.
  const variant = variantFor(body);
  const wantsWelcome = !!variant;
  const suppressBeehiivWelcome = wantsWelcome || isTonehomesLead;
  ...
  try {
    const r = await fetch(`https://api.beehiiv.com/v2/publications/${pubId}/subscriptions`, {
      ...
      body: JSON.stringify({
        ...
        send_welcome_email: !suppressBeehiivWelcome,
        utm_source: beehiivSource,
        utm_medium: beehiivMedium,
        utm_campaign: beehiivCampaign
      })
    });
